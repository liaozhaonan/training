  ...
  <div id="calendar" v-show="showCalendar">
    ...
    <div class="days">
      <ul>
        ...
        <li v-for="(dayObj, index) in days" :key="index" :class="{ 'active': index == isActiveDay }" @click="pickDay(dayObj.day, index)" >
          <!--本月-->
          <!--如果不是本月  改变类名加灰色-->
          <span class="other-month" v-if="dayObj.day.getMonth() + 1 != currentMonth">{{dayObj.day.getDate()}}</span>
          <!--如果是本月  还需要判断是不是这一天-->
            <!--今天  同年同月同日-->
            <span v-else-if="dayObj.day.getFullYear() == new Date().getFullYear() && dayObj.day.getMonth() == new Date().getMonth() && dayObj.day.getDate() == new Date().getDate()" class="today">{{ dayObj.day.getDate() }}</span>
...
export default{
  ...
  methods: {
    initData (cur) {
      ...
      this.todayIndex = this.days.findIndex(function (value, index, arr) {
        let now = new Date()
        return value.day.getDate() === now.getDate()
      })
    },
    ...
    pickDay (date, index) {
      ...
      this.$emit('pickDay', this.formatDate(date.getFullYear(), date.getMonth() + 1, date.getDate()))
      this.isActiveDay = index
    },
